         ...
         with fiona.open(shapefile_path) as shapefile:
             for feature in shapefile:
                 ...
                 for path in sat_paths:
                     #NDV, xsize, ysize, GeoT, Projection, data = GetnetCDFInfobyName(path,'data')
                     ncFile = Dataset(path,'r')
                     data = ncFile['data'][0,:,:]
                     lon = ncFile['lon'][:]
                     lat = ncFile['lat'][:]

                     data = convert_raw_to_grayscale(data)
                     data = np.nan_to_num(data)
                     scaler = MinMaxScaler(feature_range=(0,255))
                     scaler.fit(data)
                     data = scaler.transform(data)
                     area_extent=(-124.628906,29.840644,-69.609375,48.574790)
                     result = resample_data_to_geotiff(lat,lon,data,'test.tif',area_extent,bilin=True)
                     #outfile = create_geotiff(BITMAPS_DIR+bands[i]+'/data', data, NDV, xsize, ysize, GeoT, Projection)
                     #i = i+1
                     #raster_dataset, wgs_location = reproject_dataset(tif_path)
                     raster_dataset = rasterio.open('test.tif')
                     raster_ref = rasterio.open('image_ref.tif')
	         ...
	         with fiona.open(shapefile_path) as shapefile:
                for feature in shapefile:
                    ...
                    bands = ['BAND_03']
                    sat_paths = get_bands_path(end_time,start_time,bands,sat_path)
                    ...
                    band = bands[0]
                    #NDV, xsize, ysize, GeoT, Projection, data = GetnetCDFInfobyName(path,'data')
                    bmp_path = os.path.join(BITMAPS_DIR,shapefile.name,id,band)
                    ...
                    data = scaler.transform(data)
                    area_extent=(-124.628906,29.840644,-69.609375,48.574790)
                    result = resample_data_to_geotiff(lat,lon,data,os.path.join(bmp_path,'wgs84.tif'),area_extent,bilin=True)
                    #outfile = create_geotiff(BITMAPS_DIR+bands[i]+'/data', data, NDV, xsize, ysize, GeoT, Projection)
                    #i = i+1
                    #raster_dataset, wgs_location = reproject_dataset(tif_path)
                    raster_dataset = rasterio.open(os.path.join(bmp_path,'wgs84.tif'))
                    bitmap_image = rasterio.features.rasterize(
                    [(geom,255)],
                    out_shape=raster_dataset.shape,
                    transform=raster_ref.transform)
                    io.save_bitmap(os.path.join(bmp_path,'bitmap.bmp'), np.asarray(bitmap_image,dtype='uint8'), raster_dataset)
                    combine_imgs(os.path.join(bmp_path,'wgs84.tif'),os.path.join(bmp_path,'bitmap.bmp'),os.path.join(bmp_path,'overlay.tif'))
